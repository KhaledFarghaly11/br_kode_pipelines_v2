...
from pipelines.sync_members_pipeline import (
    load_json_file,
...
with DAG(
    'paymob_sync_members_automation',
    ...
    if is_incremental:
        check_last_load_date_task = PythonOperator(
            task_id=f'check_members_last_load_date',
    ...
    transform_data_task = PythonOperator(
        task_id='transform_members_data',
    ...
    if is_incremental:
        handle_new_rows_and_updates_task = PythonOperator(
            task_id=f'handle_members_new_rows',
            python_callable=handle_new_rows_and_updates,
            op_args=[transform_data_task.output, TABLE_NAME],
            provide_context=True
        )

    load_data_task = PythonOperator(
        task_id=f'load_members_data',
        python_callable=load_data_to_snowflake,
        op_args=[  
            TABLE_NAME,
            handle_new_rows_and_updates_task.output if is_incremental else transform_data_task.output,
            SCHEMA_NAME
        ],
        provide_context=True
    )
    ...
    paymob_table_info_task = PythonOperator(
        task_id=f'paymob_members_info_task',
        python_callable=paymob_sync_table_info,
        op_args=[TABLE_NAME]
    )

    if is_incremental:
        check_last_load_date_task >> extract_data_task >> transform_data_task >> handle_new_rows_and_updates_task >> load_data_task >> paymob_table_info_task
    else:
